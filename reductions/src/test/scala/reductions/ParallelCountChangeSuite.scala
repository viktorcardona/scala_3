...
class ParallelCountChangeSuite extends FunSuite {
  ...
  test("countChange should work for change 4 and coins (1,2) ") {
    def check(money: Int, coins: List[Int], expected: Int) =
      assert(countChange(money, coins) == expected,
        s"countChange($money, $coins) should be $expected")
  ...
  test("Parallel countChange with combinedThreshold should work for multi-coins") {
    def check(money: Int, coins: List[Int], expected: Int) =
      assert(parCountChange(money, coins, ParallelCountChange.combinedThreshold(money, coins)) == expected,
        s"countChange($money, $coins) should be $expected")
