...
class BlurSuite extends FunSuite {
  ...
  test("boxBlurKernel should return the correct value on an interior pixel " +
    "of a 5x5 image with radius 2") {
    val src = new Img(5, 5)
    src(0, 0) = 0;  src(1, 0) = 1;  src(2, 0) = 2;  src(3, 0) = 2;  src(4, 0) = 5
    src(0, 1) = 3;  src(1, 1) = 4;  src(2, 1) = 5;  src(3, 1) = 3;  src(4, 1) = 6
    src(0, 2) = 6;  src(1, 2) = 7;  src(2, 2) = 8;  src(3, 2) = 4;  src(4, 2) = 7
    src(0, 3) = 50; src(1, 3) = 11; src(2, 3) = 16; src(3, 3) = 5;  src(4, 3) = 8
    src(0, 4) = 15; src(1, 4) = 20; src(2, 4) = 30; src(3, 4) = 6;  src(4, 4) = 7

    assert(boxBlurKernel(src, 2, 2, 2) === 9,
      s"(boxBlurKernel(2, 2, 2) should be 9, " +
        s"but it's ${boxBlurKernel(src, 2, 2, 2)})")
  }
